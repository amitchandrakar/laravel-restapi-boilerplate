...
class SaveCandidatePartnerPreferencesRequest extends ApiFormRequest
{
    ...
    public static function rulesWithPrefix(string $prefix): array
    {
        ...
        return [
            ...
            $p . 'preferred_min_weight' => ['nullable', 'string', 'max:32'],
            $p . 'preferred_max_weight' => ['nullable', 'string', 'max:32'],
            $p . 'preferred_body_type' => ['nullable', 'string', 'max:64'],
            $p . 'preferred_complexion' => ['nullable', 'string', 'max:64'],
            ...
            $p . 'preferred_locations' => ['nullable', 'array'],
            $p . 'preferred_locations.*.country_id' => ['nullable', 'integer', Rule::exists('countries', 'id')->where('is_active', true)],
            $p . 'preferred_locations.*.state_id' => ['nullable', 'integer', Rule::exists('states', 'id')->where('is_active', true)],
            $p . 'preferred_locations.*.city_id' => ['nullable', 'integer', Rule::exists('cities', 'id')->where('is_active', true)],
